...
class BookDetailsRoute extends Component {
  ...
  renderOnSuccess = () => (
    <BookmarkContext.Consumer>
      {value => {
        const {addBookmarkItem, bookmarkList} = value
        ...
        const isBookMarked = bookmarkList.find(eachItem => eachItem.id === id)

        const renderBookMark = isBookMarked ? (
          <ImBookmark className="bookmark" />
        ) : (
          <GrBookmark className="bookmark" />
        )

        const onClickBookMark = () => {
          addBookmarkItem({...bookDetails})
        }
